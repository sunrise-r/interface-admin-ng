...
import { DomHelperService } from 'app/shared';
import {
    FrozenStructure,
    CurrentColumnInfo,
    FROZEN_POSITION,
...
export class DataTableColumnsService {
    ...
    constructor(private domHandler: DomHandler, private domHelper: DomHelperService) {}
    ...
    freeze(event: FrozenEvent, frozenStructure: FrozenStructure): FrozenStructure {
        ...
        const elementWidth = parseInt(this.domHandler.getOuterWidth(event.column), 10);
    ...
    private findContainers(element: HTMLElement): HTMLElement[] {
        const wrapper = this.domHelper.findParentByClassName(element, SCROLLABLE_WRAPPER_CLASS);
    ...
    private determineFrozenArea(container: HTMLElement): string {
        if (container.classList.contains(FROZEN_LEFT_CONTAINER_CLASS)) {
            return FROZEN_POSITION.LEFT.toString();
        }
        if (container.classList.contains(FROZEN_RIGHT_CONTAINER_CLASS)) {
            return FROZEN_POSITION.RIGHT.toString();
        }
        return 'center';
    }
